...
describe("beside: refs that land in the same atomic publish as the transaction", () => {
  ...
  test.each(["always", "on-rejection"] as const)(
    "fetch: %s reads the named refs in the SAME fetch as the store's ref and hands beside their tips",
    async (refresh) => {
      ...
      const store = await open({ repo: fixture.left, ref: MAIN, writer: "left", backend, remote: "origin", refresh })
