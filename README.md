...
Paths are git tree paths — forward slashes, no leading slash; public paths and prefixes normalize to Unicode NFC, and `keys` returns full canonical paths, sorted. Every path inside the requested Snapshot scope must be NFC, valid UTF-8, free of file/directory collisions, and a regular blob: invalid path bytes, symlink entries, and gitlinks fail loudly instead of being replaced, followed, or ignored. Values are strict UTF-8 strings in v1 — there is no binary value mode — but a tree holding a blob that is _not_ valid UTF-8 stays usable: the entry counts for `keys` and `has`, and a transaction that never touches it carries it into the next commit as the same blob object. Only reading that value fails, naming the path. Unscoped Snapshot reads and every transaction validate the whole tree's paths; a scoped read does not decode blob values outside its scope. Unpaired JavaScript surrogates always fail on write. The whole `refs/gitomic/` namespace is reserved for protocol state. `at()` accepts only a full lowercase 40- or 64-hex object id and pins it at call time (omit for the current tip); a well-formed but missing id throws on first read.
