...
type Committed = { oid: string; retries: number; kept?: "advanced" | "moved" | "locked" }
...
Snapshot reads are scoped by path prefix through `readTree(prefix)`, and a value is read by oid only when `get` asks for it, so unrelated blobs are never decoded and `oid`, `has` and `keys` never read a blob at all. A backend prefix read that matches nothing throws `GitPrefixNotFoundError` naming the prefix, repository, and commit; Snapshot turns that into its usual `undefined` / `false` / empty-array answers. Transactions never pass a prefix — the base sees every path. They are whole-tree strict on shape (every path, mode and oid of the base is listed and validated on every attempt) and lazy on values (a blob is read and decoded only when the transaction reads it, so a write costs the paths it names, not the size of the tree).
...
**`remote`:** origin becomes the decider. The default `refresh: "always"` fetches before opening and each transaction. The push is a compare-and-swap (`--force-with-lease`), never a history rewrite, so it either fast-forwards or triggers a re-run. A remote Store uses its local ref as a cache of origin: `store.head()` and `store.at()` read the kept local tip until a refresh. An unpushed local-only tip may be replaced by origin's tip. Do not point a remote Store at a ref that carries unrelated local work. Remote Readers preserve that branch. Origin is, honestly, your server; leave it out for purely local stores.

**`refresh: "on-rejection"`:** opt in when the repository keeps a valid local ref and a leased push can arbitrate a stale base. `open` and the first attempt of each `transact` read that ref and its commit object locally, so an uncontended write uses one push and no fetch. An accepted push advances the Store's kept primary ref, including a transaction with `beside` refs; raw MULTI and its other refs retain their existing local-ref behavior. The kept ref is derived state: a push whose lease origin accepted has landed, whatever the kept ref did afterward, so a kept copy shared with another process (another Store, or the CLI with `--cache-dir`) never turns a landed write into a failure. If another process moved the kept ref first, the cache update is skipped and the next attempt re-reads it; a stale kept ref costs that attempt one refused lease and a refresh. A kept ref whose lock is held (`compareAndSwap` answers `"locked"`, typically a lock left by a Git process killed mid-update) is retried for about 200 ms at the refresh, then the refresh fails before any push, naming the lock file: remove it once no Git process is running there. Gitomic never removes a lock itself. In either refresh mode, a landed publish reports what its kept ref did in `Committed.kept`: `"advanced"`, `"moved"` by another process, or `"locked"`, which the next refresh reports. If a push rejects its lease or has an unknown outcome, Gitomic fetches before searching for the transaction receipt; a rejected write then replays on the winner under the existing retry budget. `store.head()` reads the kept local ref in both modes; a later leased push decides whether it was current. A missing or unreadable kept ref or object fails with the repository and ref named; Gitomic does not silently fetch it. The default remains `"always"` for existing callers. A caller that keeps its own copy of the same remote, such as a reader's base, refreshes it with `refreshKeptCopy({ repo, ref, remote, backend })`: the same fetch, advance and lock handling a Store's refresh runs, answering the remote tip.
...
| `transact(decide, message, { also?, from? })`                           | Read events after `from` (or the whole chain), decide what to append, write it, compare-and-swap; on a race, re-run `decide` on the winner's tail after the same bound. Over 1024 events after `from` it throws rather than decide on part. |
